...
const WORKER_PATCH_STATE_KEY = Symbol.for('dotnope.workerPatchState');
const workerPatchState = globalThis[WORKER_PATCH_STATE_KEY] || {
    isEnabled: () => false,
    createProtectedOptions: (options) => options
};
globalThis[WORKER_PATCH_STATE_KEY] = workerPatchState;
...
function checkAccess(envVar, operation = 'read') {
    ...
    // Main application always has access outside eval/Function contexts.
...
function enableStrictEnv(options = {}) {
    if (isInitialized) {
        console.warn('dotnope: Already enabled - returning limited handle');
    ...
    // Propagate protection into workers created after strict env is enabled.
    patchWorkerThreads();
...
function getObserverHandle() {
    if (!observerHandle) {
        observerHandle = Object.freeze({
            getAccessStats: getAccessStats,
            disable: () => {
                const error = new Error(
                    'dotnope: Strict environment protection is already owned by another caller.\n' +
                    '\n' +
                    'Repeated enableStrictEnv() calls return a limited handle without the disable token.'
                );
                error.code = 'ERR_DOTNOPE_HANDLE_NOT_OWNER';
                throw error;
            }
        });
    }
...
function patchWorkerThreads() {
    if (!isMainThread || workerThreadsPatched) {
        return;
    }
    ...
    if (workerThreads.Worker && workerThreads.Worker.__dotnopePatched) {
        workerThreadsModule = workerThreads;
        originalWorker = workerThreads.Worker.__dotnopeOriginal;
        workerThreadsPatched = true;
        return;
    }

    const OriginalWorker = workerThreads.Worker;
    const bootstrapPath = require.resolve('./worker-bootstrap');

    class DotnopeWorker extends OriginalWorker {
        constructor(filename, options = {}) {
            const state = globalThis[WORKER_PATCH_STATE_KEY];
            const workerOptions = state && state.isEnabled()
                ? state.createProtectedOptions(options, workerThreads, bootstrapPath)
                : options;
            super(filename, workerOptions);
        }
    }

    Object.defineProperty(DotnopeWorker, '__dotnopePatched', {
        value: true
    });
    Object.defineProperty(DotnopeWorker, '__dotnopeOriginal', {
        value: OriginalWorker
    });

    workerThreads.Worker = DotnopeWorker;
    workerThreadsModule = workerThreads;
    originalWorker = OriginalWorker;
    workerThreadsPatched = true;
}

function createProtectedWorkerOptions(options, workerThreads, bootstrapPath) {
    const workerOptions = { ...options };
    const execArgv = Array.isArray(workerOptions.execArgv)
        ? workerOptions.execArgv
        : process.execArgv;

    workerOptions.execArgv = addBootstrapRequire(execArgv, bootstrapPath);

    const workerConfig = JSON.stringify(getSerializableConfig());
    if (workerOptions.env === workerThreads.SHARE_ENV) {
        process.env.DOTNOPE_WORKER_CONFIG = workerConfig;
    } else {
        workerOptions.env = {
            ...(workerOptions.env || process.env),
            DOTNOPE_WORKER_CONFIG: workerConfig
        };
    }
...
function addBootstrapRequire(execArgv, bootstrapPath) {
    const result = Array.isArray(execArgv) ? [...execArgv] : [];

    for (let i = 0; i < result.length; i++) {
        if (
            (result[i] === '--require' || result[i] === '-r') &&
            result[i + 1] === bootstrapPath
        ) {
            return result;
        }
        if (result[i] === `--require=${bootstrapPath}` || result[i] === `-r=${bootstrapPath}`) {
            return result;
        }
    }

    return ['--require', bootstrapPath, ...result];
}

function restoreWorkerThreads() {
    // Keep the wrapper installed after disable so any Worker references captured
    // before a later enableStrictEnv() call still route through auto-protection.
}
...
workerPatchState.isEnabled = () => isInitialized;
workerPatchState.createProtectedOptions = createProtectedWorkerOptions;
patchWorkerThreads();
