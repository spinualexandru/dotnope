    ...
    try {
        ...
        if (fs.existsSync(manifestPath)) {
            ...
            if (
                manifest.node &&
                (
                    manifest.node.abi !== process.versions.modules ||
                    manifest.node.platform !== process.platform ||
                    manifest.node.arch !== process.arch
                )
            ) {
                initializationError = new Error(
                    `Native addon was built for ${manifest.node.platform}/${manifest.node.arch} ABI ${manifest.node.abi}, ` +
                    `but current Node is ${process.platform}/${process.arch} ABI ${process.versions.modules}`
                );
                nativeAvailable = false;
                return false;
            }
